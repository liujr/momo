    ...
    defaultData = {
        ...
        signin: {
            ...
            defaulta: [{
                title: '已签到',
                color: '#ef002c',
                class: 'ystep-step-active',

            },
            {
                title: '今天',
                color: '#666666',
                class: 'ystep-step-undone',
    ...
    method = {
        ...
        init: {
            ...
            inputDataBind: function() {
                ...
                // 绑定input修改事件
                $(options.editor).on('input propertychange change', '[data-bind]',
                function() {
                    var $this = $(this),
                    val = $this.val(),
        ...
        editor: {
            ...
            addItem: {
                ...
                /**
                     ...
                     * @param $items
                     */
                assemble: function(itemId, data, $items) {
                    var datas = method.diyData.addItemDatas(itemId, data);
                    $items.append(template('tpl_editor_data_item_goods', datas));
                },
				 /**
                     ...
                     * @param $items
                     */
                spike: function(itemId, data, $items) {
                    var datas = method.diyData.addItemDatas(itemId, data);
                    $items.append(template('tpl_editor_data_item_goods', datas));
                },
                 ...
                 /**
                     ...
                     * @param $items
                     */
                robbuy: function(itemId, data, $items) {
                    var datas = method.diyData.addItemDatas(itemId, data);
                    $items.append(template('tpl_editor_data_item_goods', datas));
                },
				   /**
                     ...
                     * @param $items
                     */
                promgoods: function(itemId, data, $items) {
                    var datas = method.diyData.addItemDatas(itemId, data);
                    $items.append(template('tpl_editor_data_item_goods', datas));
                },
            ...
            event: {
                ...
                register: function($form, item) {
                    ...
                    this.switchHelp();
                    // 商品组件
                    if ($.inArray(item.type, ['signin']) > -1) {
                        // 切换容器
                        this.switchBox();
                    }
                    if ($.inArray(item.type, ['goods']) > -1 || $.inArray(item.type, ['assemble']) > -1 || $.inArray(item.type, ['spike']) > -1 || $.inArray(item.type, ['robbuy']) > -1 ||  $.inArray(item.type, ['promgoods']) > -1) {
                        // 切换容器
                        this.switchBox();
                        // 选择商品
                        this.selectGoods();
                ...
                inputCheckbox: function() {
                    ...
                    // 单/多选框
                    this.$form.find('input[type=checkbox], input[type=radio]').attr('checked', true);
                },
                ...
                /**
                     ...
                     */
                inputRange: function() {
                    this.$form.on('input propertychange change', 'input[type=range]',
                    function() {
                        var $this = $(this);
                ...
                btnResetColor: function() {
                    this.$form.on('click', '.btn-resetColor',
                    function() {
                        var $this = $(this);
                ...
                itemDelete: function() {
                    var _this = this;
                    // 子元素删除事件
                    _this.$form.on('click', '.item-delete',
                    function() {
                        var $this = $(this),
                        $item = $this.parent();
                ...
                switchBox: function() {
                    ...
                    $switch.find('input[data-switch]').change(function() {
                        ...
                        $switchItem.addClass('layer-hide').siblings('.' + selectClass).removeClass('layer-hide');
                    });
                },
                ...
                selectGoods: function() {
                    ...
                    $selectGoods.selectData({
                        title: '选择商品',
                        uri: 'item/lists/status/1/type/' + type,
                        duplicate: true,
                        dataIndex: 'goods_id',
                        done: function(data) {
                        ...
                        getExistData: function() {
                            ...
                            for (var i in diyData.items[itemId].data) {
                                if (diyData.items[itemId].data.hasOwnProperty(i) && typeof diyData.items[itemId].data[i]['goods_id'] !== 'undefined') existdata.push(diyData.items[itemId].data[i]['goods_id']);
                            }
        ...
        render: {
            ...
            insertDiyItem: function(type) {
                ...
                $(options.phoneMain).append(html).find('#diy-' + diyItemId).trigger('click');
            },
